...
class AccessController extends CommonController {
    ...
    public function changeRole() {
        ...
        if (IS_POST) {
            ...
            $M = M("AuthRule");
            $info = M("AuthGroup")->where("id=" . (int) $_GET['id'])->find();
            if (empty($info['id'])) {
            ...
            $access = M("AuthGroup")->field("`rules` as val")->where("`id`=" . $info['id'])->select();
            $access = explode(',', $access[0]['val']);
            $info['access'] = count($access) > 0 ? json_encode($access) : json_encode(array());

            $this->assign("info", $info);
            $datas = $M->where("level=1")->order('sort ASC')->select();
            foreach ($datas as $k => $v) {
                $map['level'] = 2;
                $map['pid'] = $v['id'];
                $datas[$k]['data'] = $M->where($map)->order('sort ASC')->select();
    ...
    private function getRole($info = array()) {
        $cat = new \Com\Category('AuthGroup', array('id', 'pid', 'title', 'fullname'));
