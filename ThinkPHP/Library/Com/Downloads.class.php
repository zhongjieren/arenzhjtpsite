...
class DownLoads extends SqlHelper {
    ...
    function downloadFile($attachName, $filename) {
        $this->setfilename($filename);
        if ($this->filecheck()) {
            $fn = array_pop(explode('/', strtr($this->filename, '\\', '/')));
            $fileExtension = strtolower(substr(strrchr($this->filename, "."), 1));
            ob_end_clean();
            ...
            //header("Content-Disposition: attachment; filename=" . $attachName . "." . $fileExtension);
